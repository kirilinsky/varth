...
CLI-only rewrite. `npx varth init` / `varth gen` / `varth gen --watch` compile a `varth.config.ts` into modern CSS — `light-dark()`, `color-scheme`, `@property`, oklch color ramps — plus a generated ~1 kB typed theme switcher (`varth.js` with a derived `varth.d.ts`, so `setTheme("drak")` is a compile error).
